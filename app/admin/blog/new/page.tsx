...
export default function AdminBlogNewPage() {
  ...
  useEffect(() => {
    blogAdminAPI.listCategories().then((r) => setCategories(r.data?.results || r.data || [])).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await blogAdminAPI.createPost(payload);
      router.push('/admin/blog');
    } catch (err: any) {
      alert(apiError(err, 'Failed to create post'));
    ...
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter flex items-center gap-3">
          <PenTool className="text-blue-600" size={34} /> New Post
        </h1>
      </div>
